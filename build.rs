...
fn main() {
    ...
    let mut code = std::fs::read_to_string("src/equinoxes_pattern.rs")
        .expect("Could not read src/equinoxes_pattern.rs");
    for gregorian_year in 1583..=2999 {
        ...
        let ts = equinoxes.get(&republican_year).unwrap_or_else(|| {
            panic!("Could not find equinox for year {republican_year} (gregorian {gregorian_year})")
        });
        code = code.replace("EQUINOX,", format!("{ts}, EQUINOX,").as_str());
    }
    code = code.replace("EQUINOX,", "");
    std::fs::write(format!("{out_dir}/equinoxes.rs"), code)
        .expect("Could not write src/equinoxes.rs");

    // Generate years.rs
    let mut code = std::fs::read_to_string("src/years_pattern.rs")
        .expect("Could not read src/years_pattern.rs");
    for gregorian_year in 1583..=2999 {
        let republican_year0 = gregorian_year - 1792;
        let republican_year = if republican_year0 >= 0 {
            republican_year0 + 1
    ...
    for gregorian_year in 1583..2999 {
        let year_start = year_starts
            .get(&gregorian_year)
            .unwrap_or_else(|| panic!("Could not find year (greg {gregorian_year})"));
        let next_year_start = year_starts
            .get(&(gregorian_year + 1))
            .unwrap_or_else(|| panic!("Could not find year (greg {gregorian_year}+1)"));
