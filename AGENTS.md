...
- ./js/README.md - Should be based on the Rust readme. Adapts Rust examples and Rust-specific stuff to the JS usage. In the introduction, states that it's a wrapper over the Rust introduction that can run using WASM. Will be shown on npmjs.com for calendrier-solar. It should state that solar time is enabled and link to the non-solar version for those who want to use mean time instead.
